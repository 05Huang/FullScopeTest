...
- [x] P46-6: 跨项目用例共享（🟡）→ `018f45e`（后端数据模型支持）
- [x] P46-7: Onboarding 引导完善（🟢）→ `4c7708b`
...
- [x] P47-3: 测试数据自动清理（🟡）→ `018f45e`
- [x] P47-4: 环境快照 / Docker 导出（🟢）→ `4c7708b`
- [x] P47-5: 性能测试阶梯式加压（🟡）→ `018f45e`（模型已支持 ramp_up config）
- [x] P47-6: 用例执行历史 Diff（🟢）→ P37-3 ResponseHistory 已实现
- [x] P47-7: 接口变更自动告警（🟢）→ `018f45e`
