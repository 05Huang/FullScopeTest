...
```sql
-- 创建用户
CREATE USER easytest WITH PASSWORD 'REDACTED_DB_PASSWORD';
...
- 重置用户密码：
```sql
ALTER USER easytest WITH PASSWORD 'REDACTED_DB_PASSWORD';
```
...
或使用第三方工具如 `pgloader`:
```bash
pgloader sqlite://easytest_dev.db postgresql://easytest:REDACTED_DB_PASSWORD@localhost/easytest_dev
```
