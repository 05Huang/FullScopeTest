...
def upgrade():
    ...
    has_report_id = 'report_id' in {c['name'] for c in inspector.get_columns('test_runs')}

    with op.batch_alter_table('test_runs', schema=None) as batch_op:
        if not has_report_id:
            batch_op.add_column(sa.Column('report_id', sa.Integer(), comment='关联报告ID', nullable=True))
        else:
