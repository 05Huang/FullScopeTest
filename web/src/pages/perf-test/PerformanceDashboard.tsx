  ...
  const buildCompareChart = (
    ...
    series: compareData.map((d, i) => {
      ...
      return {
        ...
        type: 'line',
        smooth: true,
        data: metrics.map((m: Record<string, unknown>) => m[field]),
  ...
  const compareResponseTimeOption = (() => {
    ...
    compareData.forEach((d, i) => {
      ...
      ;(opt.series as unknown[]).push({
        ...
        type: 'line',
        smooth: true,
        data: metrics.map((m: Record<string, unknown>) => m.p95_response_time),
      ...
      ;(opt.series as unknown[]).push({
        ...
        type: 'line',
        smooth: true,
        data: metrics.map((m: Record<string, unknown>) => m.p99_response_time),
        itemStyle: { color: baseColor },
        lineStyle: { width: 1, type: 'dotted' },
        symbol: 'diamond',
        symbolSize: 3,
